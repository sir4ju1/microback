...
class RestGen {
  constructor(path, model) {
    this.puralizedPath = pluralize(path)
    this.path = path
    this.model = model
    this.noAuth = []
  }
  ...
  generate () {
    var self = this
    router.get(`/${this.plurarizedPath}`, this.find.bind(this))
    router.get(`/${this.path}/:id`, this.findOne.bind(this))
    router.post(`/${this.plurarizedPath}`, body, this.create.bind(this))
    router.put(`/${this.plurarizedPath}`, body, this.update.bind(this))
    ...
    list.forEach((fun) => {
      switch (fun) {
        ...
        default:
          ...
          route.path = `/${this.plurarizedPath}/${route.path}`
          if (this.noauths && this.noauths[fun]) {
            this.noAuth.push(`/api${route.plurarizedPath}`)
          }
          switch (route.method) {
            case 'post':
            case 'put':
            case 'patch':
            case 'delete':
              router[route.method](`${route.plurarizedPath}`, body, this[fun].bind(this))
              break
            default:
              router.get(`${route.plurarizedPath}`, this[fun].bind(this))
              break
          }
          break
      }
