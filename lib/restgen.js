...
class RestGen {
  ...
  generate () {
    var self = this
    router.get(`/${this.puralizedPath}`, this.find.bind(this))
    router.get(`/${this.path}/:id`, this.findOne.bind(this))
    router.post(`/${this.puralizedPath}`, body, this.create.bind(this))
    router.put(`/${this.puralizedPath}`, body, this.update.bind(this))
    ...
    list.forEach((fun) => {
      switch (fun) {
        ...
        default:
          ...
          route.path = `/${this.puralizedPath}/${route.path}`
          if (this.noauths && this.noauths[fun]) {
            this.noAuth.push(`/api${route.puralizedPath}`)
          }
          switch (route.method) {
            case 'post':
            case 'put':
            case 'patch':
            case 'delete':
              router[route.method](`${route.puralizedPath}`, body, this[fun].bind(this))
              break
            default:
              router.get(`${route.puralizedPath}`, this[fun].bind(this))
              break
          }
          break
      }
